...
bool check_collision_with_obstacles(Snake* snake) {
    Position obstacles[] = {
        {0, 0}, {0, 1}, {0, 2}, {1, 2}, {2, 2},
        {(SCREEN_WIDTH / TILE_SIZE) - 1, (SCREEN_HEIGHT / TILE_SIZE) - 1},
        {(SCREEN_WIDTH / TILE_SIZE) - 2, (SCREEN_HEIGHT / TILE_SIZE) - 1},
        {(SCREEN_WIDTH / TILE_SIZE) - 3, (SCREEN_HEIGHT / TILE_SIZE) - 1},
        {(SCREEN_WIDTH / TILE_SIZE) - 3, (SCREEN_HEIGHT / TILE_SIZE) - 2},
        {(SCREEN_WIDTH / TILE_SIZE) - 3, (SCREEN_HEIGHT / TILE_SIZE) - 3}
    };

    for (int i = 0; i < sizeof(obstacles) / sizeof(obstacles[0]); i++) {
        if (snake->segments[0].x == obstacles[i].x && snake->segments[0].y == obstacles[i].y) {
            return true;
        }
    }
    return false;
...
bool game_loop(SDL_Renderer* renderer, TTF_Font* font, Mix_Chunk* eat_sound, Mix_Chunk* collision_sound, SDL_Texture* bg_texture, SDL_Texture* food_texture, SDL_Texture* snake_texture, SDL_Texture* game_over_texture) {
    ...
    while (running) {
        ...
        if (check_collision_with_obstacles(&snake)) {
            Mix_PlayChannel(-1, collision_sound, 0);

            SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255);
            SDL_RenderClear(renderer);
            draw_text(renderer, font, "You hit an obstacle!", (SDL_Color){255, 255, 255, 255}, SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 - 50);
            draw_text(renderer, font, "Press Y or N", (SDL_Color){255, 255, 255, 255}, SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2);
        ...
        for (int i = 0; i < sizeof(obstacles) / sizeof(obstacles[0]); i++) {
            SDL_Rect obstacle_rect = {obstacles[i].x * TILE_SIZE, obstacles[i].y * TILE_SIZE, TILE_SIZE, TILE_SIZE};
